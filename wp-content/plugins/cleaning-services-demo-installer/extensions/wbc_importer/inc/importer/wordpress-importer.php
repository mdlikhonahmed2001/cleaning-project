...
class WP_Import extends WP_Importer {
	...
	var $same_image_at_all=true;
	var $stock_images = array();
	var $temp_images = array(
        'post' => 'blog.jpg',
        'brands' => 'brand.jpg',
        'testimonials' => 'testimonial.jpg',
        'gallery' => 'gallery.jpg',
        'product' => array(
            'product1.jpg',
            'product2.jpg',
	 ...
	 */
	private $_img_id_pattern_for_shortcode = '#(( promo__image)|( image)|( back_image))="(\d+)"#';
	private function _replace_img_id_in_shortcodes($match){
        $bimages = $this->stock_images['attachment'];
        $bimage = array_shift($bimages);
        return ' '.$match[1].'="'.$bimage.'"';
    }
	function dispatch() {
		$this->header();
...
error_reporting(0);
		...
		/*if($this->same_image_at_all){
            $this->import_stock_images_one_at_all();
        }else{
            $this->import_stock_images();
        }*/
	...
	private function import_stock_images_one_at_all(){
        $path = WP_PLUGIN_DIR.'/'.'car-repair-demo-installer'.'/extensions/wbc_importer/dummy-img/';
		
        $attachment_id = $this->create_the_attachment($path.$this->temp_images['post']);
        foreach($this->temp_images as $key => $filename){
            if(is_array($filename)){
                $this->stock_images[$key] = array();
                foreach($filename as $singlefile){
                    $this->stock_images[$key][] = $attachment_id;
                }
            }else{
                $this->stock_images[$key] = $attachment_id;
            }
    ...
    private function import_stock_images(){
		$path = WP_PLUGIN_DIR.'/'.'car-repair-demo-installer'.'/extensions/wbc_importer/dummy-img/';
        $temp_images = $this->temp_images;
        foreach($temp_images as $key => $filename){
            if(is_array($filename)){
                $this->stock_images[$key] = array();
                foreach($filename as $singlefile){
                    $attachment_id = $this->create_the_attachment($path.$singlefile);
                    $this->stock_images[$key][] = $attachment_id;
                }
            }else{
                $attachment_id = $this->create_the_attachment($path.$filename);
                $this->stock_images[$key] = $attachment_id;
            }
        }
    }
...
private function create_the_attachment($filename){
        ...
        copy($filename, $uploadpath.$image);

        $filename = $uploadpath.$image;
        
        
        
        // Prepare an array of post data for the attachment.
        $attachment = array(
            'guid'           => $wp_upload_dir['url'] . '/' . $image, 
		...
		foreach ( $this->terms as $term ) {
			...
			if ( ! is_wp_error( $id ) ) {
				if ( isset($term['term_id']) )
					...
					if(function_exists('set_wcm_sds_brand_meta') && $term['term_taxonomy'] == 'brands'){
                        
                        $term_id = $id['term_id'];
                        $tt_id = $id['term_taxonomy_id'];
                        $meta_value = $this->stock_images['brands'];
                        
                        set_wcm_sds_brand_meta(array(
	...
	function process_posts() {
		...
		foreach ( $this->posts as $post ) {
			...
			if ( $post['post_type'] == 'attachment' ) {
				//if($post['post_title']!="logo" && $post['post_title']!="logo-mobile"){
					//continue;
				//}
			}
			...
			if ( $post_exists && get_post_type( $post_exists ) == $post['post_type'] ) {
				...
				if ( isset( $this->author_mapping[$author] ) )
					...
					//$post_content = preg_replace_callback($this->_img_id_pattern_for_shortcode, array($this, '_replace_img_id_in_shortcodes'), $post['post_content']);
				...
				foreach ( $post['postmeta'] as $meta ) {
					...
					if ( $key ) {
						...
						if($key == '_product_image_gallery'){
                            $pimages = $this->stock_images['product'];
                            array_shift($pimages);
                            $pimages = implode(',', $pimages);
                            $value = $pimages;
                        }elseif($key == '_thumbnail_id'){
                            if(isset($this->stock_images[$post['post_type']])){
                                if(is_array($this->stock_images[$post['post_type']])){
                                    $value = $this->stock_images[$post['post_type']];
                                    $value = array_shift($value);
                                }else{
									
									//if($key!='logo' && $key!='logo-mobile'){
										$value = $this->stock_images[$post['post_type']];
									//}
